<script lang="ts">
	...
	const shouldShowContextSwitcher = $derived(true);
	...
	async function handleToggleWebsiteMode() {
		...
		try {
			...
			const response = await api.post<any>('/settings/website-mode', { enabled: newValue });
			...
			if (response.data?.data?.success) {
				...
				toastStore.error(
					response.data?.data?.message || response.data?.message || 'Failed to toggle website mode'
				);
			}
		} catch (error: any) {
			toastStore.error(
...
<aside class="bg-background fixed inset-y-0 left-0 flex w-64 flex-col border-r">
	<div class="flex h-16 items-center justify-between border-b px-6">
		<div
			class="flex cursor-pointer flex-col justify-center transition-opacity hover:opacity-80"
			onclick={() => goto('/')}
			role="button"
			tabindex="0"
			onkeydown={(e) => {
	...
	<div class="space-y-2 border-t p-4">
		{#if isGod}
			...
			>
				...
				<div class="ml-auto">
					<div
						class="relative h-4 w-8 rounded-full {websiteMode ? 'bg-primary' : 'bg-muted'}"
						role="switch"
						aria-checked={websiteMode}
					>
						<div
							class="absolute top-0.5 left-0.5 h-3 w-3 rounded-full bg-white transition-transform {websiteMode
								? 'translate-x-4'
								: ''}"
						></div>
					</div>
