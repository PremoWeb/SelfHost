<script lang="ts">
	...
	// 1) Logged in → always dashboard. 2) Not logged in + website mode on → website (landing). 3) Not logged in + website mode off → redirect to login or message.
	const websiteMode = $derived((data as any)?.websiteMode || false);
	const isUnauth = $derived(!$authStore.isAuthenticated && !$authStore.isLoading);
	const shouldShowLanding = $derived($authStore.isAuthenticated ? false : websiteMode);
	const shouldRedirectToLogin = $derived(isUnauth && !websiteMode);

	// Modal state for shallow routing
	let onboardingModalOpen = $state(false);

	// When not logged in and website mode off → redirect to login
	$effect(() => {
...
{#if shouldRedirectToLogin}
	<!-- Website mode off + not logged in: show message and redirect to login -->
	<div class="from-background to-muted/20 flex min-h-screen items-center justify-center bg-linear-to-b">
...
{:else if shouldShowLanding}
	<!-- Website mode on + not logged in: full landing (website) -->
	{#if websiteMode}
		<PublicHeader />
	{/if}
	<div class="from-background to-muted/20 min-h-screen bg-linear-to-b">
		...
		<!-- CTA Section -->
		<section class="container mx-auto px-4 py-20">
			<div class="bg-card mx-auto max-w-4xl rounded-lg border p-12 text-center">
				...
				<div class="flex flex-col gap-4 sm:flex-row sm:justify-center">
					...
					<Button size="lg" variant="outline" onclick={() => goto('/docs')}>Read the Docs</Button>
				</div>
			</div>
		</section>
	</div>
...
{:else}
	...
	<div class="space-y-8">
		...
		{:else}
			<!-- Dashboard: shown when logged in, or while loading (website mode off + unauthenticated redirects) -->
										...
										<div class="space-y-1">
											<div class="text-muted-foreground flex justify-between text-[10px] uppercase">
												<span class="flex items-center gap-1"
													><Database class="size-2.5" /> RAM</span
												>
					...
					<Card.Root class="bg-muted/30 border-dashed">
						<Card.Content class="p-4 text-center">
							<p class="text-muted-foreground mb-3 text-xs">
								The SelfHost Cloud Agent sends encrypted heartbeats every 5 seconds to ensure
