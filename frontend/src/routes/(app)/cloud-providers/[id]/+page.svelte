...
{#snippet teamKeyRow(key: any)}
	{@const isInstalled = (data as any).providerSshKeys.some((pk: any) => pk.name === key.name)}
	<TableRow>
		<TableCell class="font-medium">{key.name}</TableCell>
		<TableCell class="text-right">
			{#if isInstalled}
...
<div class="space-y-6">
	...
	<div class="space-y-4">
		<div class="flex items-center justify-between">
			...
			<Badge variant="secondary">{data.instances.length} Active</Badge>
		</div>
						...
						<TableRow>
							<TableCell class="font-medium">
								<div class="space-y-1">
									...
									<code class="text-muted-foreground font-mono text-[10px]">{instance.main_ip}</code
									>
								</div>
							</TableCell>
							<TableCell>
		...
		<Card.Root>
			...
			<Card.Content>
				<Table>
					...
					<TableBody>
						{#each (data as any).providerSshKeys as key}
		...
		<Card.Root>
			...
			<Card.Content>
				<Table>
					...
					<TableBody>
						{#each (data as any).teamPrivateKeys as key}
							{@render teamKeyRow(key)}
						{/each}
					</TableBody>
				</Table>
			</Card.Content>
			...
			<div class="grid grid-cols-2 gap-4">
				<div class="space-y-2">
					...
					>
						...
						{#each data.meta.regions as region}
				...
				<div class="space-y-2">
					...
					>
						...
						{:then availablePlans}
							{@const activePlans =
								!selectedRegion || availablePlans.length === 0 ? data.meta.plans : availablePlans}
...
<Dialog.Root bind:open={showSaveTemplateDialog}>
	<Dialog.Content class="sm:max-w-md">
		<Dialog.Header>
			...
			<Dialog.Description>Save current configuration for quick deployment later</Dialog.Description>
		</Dialog.Header>
		<form
			method="POST"
			action="?/saveTemplate"
