...
<Dialog.Root bind:open={showAddModal}>
	<Dialog.Content>
			...
			use:enhance={() => {
				return async ({ result }) => {
					if (result.type === 'success') {
						...
						toastStore.error((result.data?.message as string) || 'Failed to connect provider');
		...
		>
			...
			{/if}
			<Dialog.Footer>
				<Button type="submit" class="w-full">Connect Provider</Button>
			</Dialog.Footer>
		</form>
	</Dialog.Content>
</Dialog.Root>
		...
		<AlertDialog.Footer>
			...
			>
				...
				<input
					type="hidden"
					name="type"
					value={providerToDelete?.deleteType === 'cloudflare_access'
						? 'cloudflare_access'
						: 'provider'}
				/>
				<AlertDialog.Action
