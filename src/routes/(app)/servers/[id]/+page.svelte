<script lang="ts">
		...
		const interval = setInterval(async () => {
			try {
				...
				if (response.success && response.data) {
					...
					server.healthCpu = updated.healthCpu;
					server.healthMemory = updated.healthMemory;
					server.healthDisk = updated.healthDisk;
					server.healthUpdatedAt = updated.healthUpdatedAt;
				}
			} catch (err) {}
		}, 5000);
	...
	let callbackUrl = $state(
		untrack(() => (data.tunnelUrl ? data.tunnelUrl.replace('https://', 'wss://') : ''))
	);
	...
	async function handleInstallAgent() {
		...
		installProgress = [
			...
			{ step: 'uploading', message: 'Uploading SelfHost Agent source...', status: 'pending' },
			{ step: 'installing_bun', message: 'Installing Bun runtime...', status: 'pending' },
			{ step: 'starting', message: 'Starting SelfHost Agent service...', status: 'pending' }
		];
						...
						if (line.startsWith('data: ')) {
							try {
								...
								if (index !== -1) {
									...
									if (data.tunnelUrl) {
										try {
											// Use the existing forceUpdateService from server.remote.ts
